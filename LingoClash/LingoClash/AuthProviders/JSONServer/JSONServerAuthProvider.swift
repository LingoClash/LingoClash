...
class JSONServerAuthProvider: AuthProvider {
    ...
    func login(params: [String: Any]) -> Promise<Void> {
        guard let email = params[Configs.email] as? String, let password = params[Configs.password] as? String else {
        ...
        return FetchUtilities.fetchData(with: request).done { result in
            ...
            try KeychainManager.save(service: self.apiURL, account: Configs.accessToken, value: accessToken.data(using: .utf8) ?? Data())
        }
    }
    ...
    func logout() -> Promise<Void> {
        ...
        do {
            try KeychainManager.delete(service: self.apiURL, account: Configs.accessToken)
        } catch {
